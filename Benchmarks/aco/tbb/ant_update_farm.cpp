...
void update_part( unsigned int best_t, unsigned int *best_result, unsigned int i) {
  unsigned int j;
...
/********* Intel TBB specific part ***********/
class CPU_Update_Farm_Component {
private:
  unsigned int best_t;
  unsigned int *best_result;
public:
  void operator()(const blocked_range<size_t> &r) const {
    for (size_t i=r.begin(); i!=r.end(); i++) {
      update_part(best_t, best_result, i);
    }
  }
  CPU_Update_Farm_Component (unsigned int best_t, unsigned int *best_result) :
    best_t(best_t), best_result(best_result) {};
};
/**********************************************/
...
int main(int argc, char **argv) {
  ...
  char *fname;
  unsigned int i, j;
  int nr_cpu_w, do_chunking, min_chunk_size = 1, min_chunk_size_update = 1;
  ...
  // min_chunk_size_update = num_jobs / nr_cpu_w;
  // }
  /***************************************/
  ...
  for (j=0; j<num_iter; j++) {
    ...
    /******* Intel TBB farm ********/
    parallel_for(blocked_range<size_t>(0, num_jobs, min_chunk_size_update), CPU_Update_Farm_Component(best_t, best_result));
    /*******************************/
