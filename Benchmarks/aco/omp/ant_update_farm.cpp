...
#pragma omp parallel for shared(cost,min_chunk_size) private(i) schedule(static, min_chunk_size)
    for (i=0; i<num_ants; i++) {
      cost[i] = solve(i);
    }
...
#pragma omp parallel for shared(cost,best_t,best_result,min_chunk_size_update) private(i) schedule(static, min_chunk_size_update)
    for (i=0; i<num_jobs; i++) {
